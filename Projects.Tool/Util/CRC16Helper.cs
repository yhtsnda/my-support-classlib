...
namespace Projects.Tool.Util
{
    public class CRC16Helper
    {
        ...
        private static ushort CheckCrc(ushort crc, byte cp)
        {
        ...
        /// 添加CRC校验字
        /// </summary>
        /// <param name="buf">信息串</param>
        /// <param name="n">不包括校验字的串总长度</param>
        public static void AddCRC(ref byte[] buf, byte n)
        {
            ushort crc16 = 0;
            byte i;
            //n个数据的CRC校验
            for (i = 0; i < n; i++)
            {
                crc16 = CheckCrc(crc16, buf[i]);
            }
            buf[i] = (byte)(crc16 >> 8);
            buf[i + 1] = (byte)(crc16 & 0xff);
        }
        ...
        public static bool ValidateCRC(ref byte[] bufout, byte count)
        {
            ushort crc16 = 0;
            byte i;
            for (i = 0; i < (count - 2); i++)
                crc16 = CheckCrc(crc16, bufout[i]);
