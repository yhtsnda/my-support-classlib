...
namespace Projects.Tool.Diagnostics
{
    public static class ProfilerUtil
    {
        ...
        static ProfilerUtil()
        {
            string filter = ConfigurationManager.AppSettings["nd.profiler.filters"];
            filters = filter != null ? filter.Split(',') : new string[] { "System", "BLToolkit", "Projects.Tool", "PostSharp" };
        }
